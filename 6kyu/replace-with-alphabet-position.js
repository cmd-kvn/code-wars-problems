...
module.exports = function alphabetPosition(text) {
    const alphabet = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 
        'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'];
    const textLetters = text.toLowerCase().replace(/[^a-z]*/gi, '');

    return textLetters.split('').map(ltr => {
        return alphabet.indexOf(ltr) + 1;   // add 1 to offset 0-index; equal to alphabet position
    }).join(' ');
};
